...
mod tests {
    ...
    fn setup() -> (TempDir, Arc<dyn Memory>, Arc<Mutex<SyncEngine>>) {
        ...
        let sync = Arc::new(Mutex::new(SyncEngine::new(tmp.path(), true, None).unwrap()));
        (tmp, mem, sync)
    }
    ...
    async fn two_device_roundtrip_sync() {
        ...
        let sync_a = Arc::new(Mutex::new(
            SyncEngine::new(tmp_a.path(), true, None).unwrap(),
        ));
        let synced_a = SyncedMemory::new(mem_a, sync_a.clone());

        let mem_b: Arc<dyn Memory> = Arc::new(SqliteMemory::new(tmp_b.path()).unwrap());
        let sync_b = Arc::new(Mutex::new(
            SyncEngine::new(tmp_b.path(), true, None).unwrap(),
        ));
