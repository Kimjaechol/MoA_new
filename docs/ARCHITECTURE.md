...
【최초 판단】 사용자의 로컬 디바이스가 온라인인가? (DeviceRouter)
    ...
    │             → ★ 하이브리드 릴레이 (hybrid_relay + proxy token)
    │             → Railway가 단기 프록시 토큰 발급 (15분 TTL)
    │             → ⛔ 운영자 API key는 절대 디바이스에 전송하지 않음
    │             → 로컬 디바이스에서 처리:
    │                 • LLM 호출 → 프록시 토큰으로 Railway /api/llm/proxy 경유
    │                   (Railway 서버에서 운영자 key 주입 → LLM 호출)
    │                 • 도구 실행 → 로컬 도구 API key 사용 ✅
    │                 • 설정/config → 로컬 설정 적용 ✅
    │             → 💰 크레딧 2.2× 차감 (서버 측에서 자동 차감)
    │
    │             ※ 하이브리드 릴레이 실패 시에만 ▼
    │
...
| **웹채팅** | ✅ 온라인 | ❌ 없음 | Railway → 디바이스(프록시 토큰) → `/api/llm/proxy` → LLM | 로컬 key ✅ | 크레딧 2.2× |
