...
> **경로**: 채널 플랫폼 → 웹훅 → Railway → agent loop → 채널 응답
> **코드**: `src/gateway/mod.rs` (각 채널별 핸들러)
>
> **핵심 특성**: 채널은 웹훅 기반이므로 **반드시 공개 HTTPS 엔드포인트**를
> 경유해야 합니다. 따라서 **항상 Railway에서 처리**되며, 로컬 디바이스로
> 릴레이하지 않습니다.

```
사용자 (카카오톡/텔레그램/디스코드/WhatsApp/Slack 등)
    │
    │ 메시지 전송 (예: 카카오톡 채팅방에서 "오늘 날씨 어때?")
    │
    ▼
채널 플랫폼 서버 (카카오/텔레그램/디스코드)
    │
    │ 웹훅 POST 요청 (채널 플랫폼 → Railway)
    │ (예: POST /webhook/whatsapp, /webhook/telegram 등)
    │
    ▼
╔═══════════════════════════════════════════════════════════════╗
║  Railway 서버 — 웹훅 수신 및 검증                             ║
╚═══════════════════════════════════════════════════════════════╝
    │
    │ 1. 웹훅 서명 검증 (X-Hub-Signature-256 등)
    │    → 채널별 app_secret/signing_secret으로 검증
    │    → 위조된 요청 차단
    │
    │ 2. 채널 메시지 파싱 (parse_webhook_payload)
    │    → ChannelMessage 구조체로 변환
    │    → reply_target (응답 대상) 추출
    │
    │ 3. 사용자 허용 목록 확인 (allowed_numbers 등)
    │    → 허용되지 않은 사용자 차단
    │
    ▼
╔═══════════════════════════════════════════════════════════════╗
║  Agent Loop 실행 (Railway 서버에서)                            ║
║                                                               ║
║  run_gateway_chat_with_tools() → process_message_with_session ║
╚═══════════════════════════════════════════════════════════════╝
    │
    ├─ LLM 호출: Railway config의 API key 사용
    │            (ADMIN_*_API_KEY 환경변수 또는 config.api_key)
    │
    ├─ 도구 실행: Railway 서버의 도구 설정 사용
    │            (web_search, browser 등 — Railway config 기반)
    │            ⚠️ 사용자의 로컬 도구 key는 사용되지 않음
    │
    └─ 메모리: Railway의 SQLite 메모리에 대화 저장
              (session_id로 채널별 분리)
    │
    ▼
╔═══════════════════════════════════════════════════════════════╗
...
╚═══════════════════════════════════════════════════════════════╝
    ...
    │ → 카카오톡/텔레그램/디스코드 API로 응답 전송
    │ → 사용자의 채팅방에 응답 표시
    │
    ▼
💰 크레딧 2.2× 차감

※ 채널채팅에서는 로컬 디바이스 릴레이를 시도하지 않습니다.
  채널 플랫폼의 웹훅 구조상, Railway가 직접 응답해야 하기 때문입니다.
```

**채널별 인증 정보 (운영자가 Railway에 사전 설정)**:

| 채널 | 웹훅 경로 | 인증 토큰 출처 | 서명 검증 |
|------|----------|--------------|----------|
| **WhatsApp** | `/webhook/whatsapp` | `config.channels.whatsapp.access_token` | X-Hub-Signature-256 (app_secret) |
| **Telegram** | `/webhook/telegram` | `config.channels.telegram.bot_token` | 토큰 URL 경로 검증 |
| **Discord** | `/webhook/discord` | `config.channels.discord.bot_token` | Ed25519 서명 검증 |
| **Slack** | `/webhook/slack` | `config.channels.slack.bot_token` | signing_secret HMAC |
| **카카오톡** | `/webhook/kakao` | `config.channels.kakao.rest_api_key` | — |
| **QQ** | `/webhook/qq` | `config.channels.qq.app_secret` | 서명 검증 |
| **Linq** | `/webhook/linq` | `config.channels.linq.api_token` | signing_secret |
| **WATI** | `/webhook/wati` | `config.channels.wati.api_token` | — |

**구현 파일**:

| 단계 | 파일 | 핵심 함수 |
|------|------|----------|
| 웹훅 라우팅 | `src/gateway/mod.rs` | 채널별 `.route()` 등록 (line 1104+) |
| WhatsApp 처리 | `src/gateway/mod.rs` | `handle_whatsapp_message()` |
| Telegram 처리 | `src/channels/telegram.rs` | `TelegramChannel::listen()` |
| Discord 처리 | `src/channels/discord.rs` | `DiscordChannel::listen()` |
| 채널 초기화 | `src/gateway/mod.rs` | gateway startup (line 699+) |
| Agent Loop | `src/gateway/mod.rs` | `run_gateway_chat_with_tools()` |
| 응답 전송 | `src/channels/traits.rs` | `Channel::send()` |

**채널채팅의 핵심 특성**:
- **운영자가 모든 것을 사전 설정** — 사용자는 채팅방에서 메시지만 보내면 됨
- **로컬 디바이스 릴레이 없음** — 항상 Railway에서 처리
- **도구 실행도 Railway** — Railway의 config/도구 설정 사용
- **비용은 운영자 부담** (또는 크레딧으로 사용자에게 전가)
- **컴맹 친화적** — 앱 설치 불필요, 채팅방 추가만 하면 즉시 사용 가능
...
| **① 앱채팅** | 로컬 LLM key ❌ | ProxyProvider → Railway `/api/llm/proxy` → LLM | 로컬 key ✅ | 💰 2.2× |
| **② 웹채팅** | 디바이스 온라인 + LLM key ✅ | Railway → 디바이스 릴레이 → LLM 직접 | 로컬 key ✅ | **무료** |
| **② 웹채팅** | 디바이스 온라인 + LLM key ❌ | Railway → 디바이스(proxy token) → `/api/llm/proxy` | 로컬 key ✅ | 💰 2.2× |
| **② 웹채팅** | 디바이스 오프라인 | Railway → LLM 직접 (운영자 key) | Railway 설정 ⚠️ | 💰 2.2× |
| **③ 채널채팅** | 항상 Railway 처리 | Railway → LLM 직접 (운영자 key) | Railway 설정 ⚠️ | 💰 2.2× |

> **도구 실행 열 설명**:
> - **로컬 key ✅**: 디바이스에 설정된 각 도구의 API key(웹검색, 브라우저, Composio 등)와 로컬 설정이 사용됨
> - **Railway 설정 ⚠️**: Railway 서버의 도구/설정만 사용 가능 (사용자의 로컬 도구 key 미사용)

> **핵심 차이**:
> - ①②에서 로컬 LLM key 없는 경우 → **도구는 여전히 로컬 key 사용** (ProxyProvider가 LLM만 프록시)
> - ②③에서 디바이스 오프라인/채널 → 도구도 Railway에서 실행 (로컬 key 접근 불가)
