...
【최초 판단】 사용자의 로컬 디바이스가 온라인인가? (DeviceRouter)
    ...
    │         │   → ✅ 무료 (모든 key가 로컬)
    │         │
    │         └─ 로컬에 LLM API key 없음 (또는 프로브 타임아웃)
    │             → ★ 하이브리드 릴레이 (message_with_llm_key)
    │             → Railway의 운영자 LLM key를 디바이스에 주입하여 전송
    │             → 로컬 디바이스에서 처리:
    │                 • LLM 호출 → 주입된 운영자 key 사용
    ...
    │             ※ 하이브리드 릴레이 실패 시에만 ▼
    │
    └─ NO → Railway에서 전체 처리 (LLM + 도구 모두 Railway)
            → ⚠️  로컬 도구 API key 미사용
            → 💰 크레딧 2.2× 차감
...
| 채팅 방식 | 로컬 디바이스 | 로컬 LLM Key | LLM 호출 경로 | 도구 실행 | 비용 |
|-----------|-------------|-------------|--------------|----------|------|
| **앱채팅** | — (로컬 실행) | ✅ 있음 | 디바이스 → LLM 직접 | 로컬 key ✅ | **무료** |
| **앱채팅** | — (로컬 실행) | ❌ 없음 | 디바이스 → Railway → LLM | 로컬 key ✅ | 크레딧 2.2× |
| **웹채팅** | ✅ 온라인 | ✅ 있음 | Railway → 디바이스 → LLM | 로컬 key ✅ | **무료** |
| **웹채팅** | ✅ 온라인 | ❌ 없음 | Railway → 디바이스(운영자LLM key 주입) → LLM | 로컬 key ✅ | 크레딧 2.2× |
| **웹채팅** | ❌ 오프라인 | — | Railway → LLM | Railway ⚠️ | 크레딧 2.2× |
| **채널채팅** | — (해당 없음) | — | 채널 → Railway → LLM | Railway ⚠️ | 크레딧 2.2× |

> **도구 실행 열 설명**:
> - **로컬 key ✅**: 디바이스에 설정된 각 도구의 API key(웹검색, 브라우저, Composio 등)와 로컬 설정이 사용됨
> - **Railway ⚠️**: 디바이스가 오프라인이므로 Railway 서버의 도구/설정만 사용 가능 (로컬 도구 key 미사용)
...
| 하이브리드 릴레이 (운영자 LLM key 주입) | `src/gateway/ws.rs` | `try_relay_to_local_device_with_key()` |
