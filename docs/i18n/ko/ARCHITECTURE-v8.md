# MoA — 아키텍처 v8 (테마별 재구성판, 한국어)

> **버전**: v8.0 (테마별 재구성 — Thematic Restructure)
> **상태**: Living document — 영문 `docs/ARCHITECTURE-v8.md` 의 한국어 대응본.
> **대상 독자**: AI 리뷰어, 기여자, 유지보수자, 제품 · 비즈니스 기획자
>
> **본 문서의 위치와 관계**:
>
> - 영문 v8 원본: [`../../ARCHITECTURE-v8.md`](../../ARCHITECTURE-v8.md)
> - 영문 구 원본 (순차 번호 §1~§16): [`../../ARCHITECTURE.md`](../../ARCHITECTURE.md)
> - 본 문서(한국어 v8) — 동일 내용을 다음 13 개 테마로 재구성:
>   §1 목적 / §2 기능 / §3 장점·차별점 / §4 구조와 아키텍처 /
>   §5 SLM+LLM+Tool+Skill 유기적 로직 / §6 카테고리+프리셋+워크플로우 /
>   §7 작동 원리 / §8 사용 도구 / §9 채팅 방법 /
>   §10 First Brain / §11 Second Brain / §12 실시간 동기화 / §13 부록
>
> **번역 상태 (Translation Status, 2026-04-20 갱신)**:
> - ✅ **완전 한국어화된 섹션**: 헤더 · 목차 · §1 (목적) · §2 (기능) · §3 (장점·차별점) · §4 (구조·아키텍처 — Deployment · System Arch · Tech Stack) · §5 (SLM+LLM+Tool+Skill Orchestration) · §6 일부 (카테고리·음성·코딩·샌드박스 헤더) · §7 (작동 원리 — Browser Daemon · Design Principles · Risk Tiers) · §8 (도구 카탈로그) · §9 (채팅 방법 — App/Channel/Web) · §11 일부 (Dual-Brain · Vault 원본 Korean 보존) · §12 (Sync Journal v3 원본 Korean 보존) · §13.1 (Config) · §13.2 (Patent Areas) · §13.3 (Roadmap 헤더) · §13.5 (For AI Reviewers).
> - 🟡 **헤더만 한국어화, 본문 영문 유지**: §13.4 (15A Implementation Details — 구현 상세는 파일 경로·함수명 사실성 보존 위해 영문 원문 유지; 각 하위 섹션 제목은 한국어로 번역됨).
> - 🔒 **원문 영문 그대로 유지 (의도적)**: 모든 코드 블록 · 파일 경로 (`src/**`) · 트레이트명 · 함수명 · 상수명 · CLI 커맨드 · 커밋 해시 · 테스트 이름 — 사실성 보존이 우선이므로 번역하지 않음.
>
...
1. [MoA의 목적 (Purpose)](#1-moa의-목적-purpose)
2. [제공 기능 개요 (Features)](#2-제공-기능-개요-features)
3. [장점 및 차별점 (Advantages)](#3-장점-및-차별점-advantages)
4. [시스템 구조와 아키텍처 (Structure & Architecture)](#4-시스템-구조와-아키텍처-structure--architecture)
5. [SLM + LLM + Tool + Skill 유기적 로직 (Orchestration Logic)](#5-slm--llm--tool--skill-유기적-로직-orchestration-logic)
6. [카테고리 + 프리셋 + 워크플로우 (Categories, Presets, Workflows)](#6-카테고리--프리셋--워크플로우-categories-presets-workflows)
7. [작동 원리 (Operating Principles)](#7-작동-원리-operating-principles)
8. [사용 도구 카탈로그 (Tool Catalog)](#8-사용-도구-카탈로그-tool-catalog)
9. [채팅 방법 — 앱 / 웹 / 채널 (Chat Methods)](#9-채팅-방법--앱--웹--채널-chat-methods)
10. [기억 아키텍처 — First Brain](#10-기억-아키텍처--first-brain)
11. [기억 아키텍처 — Second Brain](#11-기억-아키텍처--second-brain)
12. [복수 디바이스 실시간 동기화 (Multi-Device Sync)](#12-복수-디바이스-실시간-동기화-multi-device-sync)
13. [부록 — Config · Patents · Roadmap · 구현 감사 · For AI Reviewers](#13-부록--config--patents--roadmap--구현-감사--for-ai-reviewers)

---

## 1. MoA의 목적 (Purpose)

이 섹션은 MoA 가 **무엇을 위해 만들어진 제품인지**, 그리고 **어떤 사용자를 대상으로** 하는지를 정의합니다.
원본 ARCHITECTURE.md §1 Product Vision, §4 Target Users 를 이 테마 아래에 통합했습니다.

<!-- CHUNK: 01_vision (원본 §1 Product Vision) -->
### 1. Product Vision
...
<!-- CHUNK: 07_target_users (원본 §4 Target Users) -->
...
MoA 가 제공하는 기능은 크게 **대화·응답**, **음성·통역**, **문서·Vault**, **코딩**, **미디어 생성**, **일정 관리**, **멀티 디바이스 연속성** 7 개 축으로 묶입니다. 본 섹션은 각 기능의 **진입점**만 요약하고, 내부 동작·로직·코드 위치는 뒤 섹션에서 상세히 다룹니다.

### 2.1 기능 카탈로그 (요약)

| 기능 축 | 사용자 관점 기능 | 내부 구현 위치 | 상세 섹션 |
|---|---|---|---|
...
| **기억(Second Brain)** | Vault 허브 노트·wikilink 그래프·compiled_truth | `src/memory/sqlite.rs`, `src/vault/**` | §11 |
| **실시간 동기화** | 여러 기기 간 기억·대화·설정을 5분 TTL 릴레이로 E2E 동기화 | `src/sync/**`, `src/security/**` | §12 |
| **스킬 자기 학습** | 이용자 수정·반복 패턴을 관찰 → 검증 → 적용 → 피드백 | `src/skills/**`, `src/user_model/**`, `src/session_search/**` | §5.5 |

### 2.2 "왜 이 기능들이 한 앱에 모여있는가"

- **단일 에이전트 루프**가 모든 카테고리를 처리하므로, 음성으로 시작한 작업을 문서로 이어가고, 같은 대화 맥락을 채널에서 이어받을 수 있습니다 (§5, §10 참조).
- **하나의 기억 저장소**(First + Second Brain)가 모든 기능의 컨텍스트를 공유 — 회의 녹취 중 인용한 판례를 나중에 코딩 세션에서도 불러올 수 있습니다 (§10, §11 참조).
- **한 번의 로그인**으로 데스크톱·모바일·웹·채널 채팅이 모두 동일한 사용자 공간을 공유 (§4, §12 참조).
...
| 채팅 채널 | 각자 별개 | **앱 + 웹 + 30 + 채널** 하나의 에이전트 루프가 처리 (§9) |
| 문서 편집 | 텍스트 입력만 | **2-Layer 편집기** (읽기 전용 iframe viewer + Tiptap WYSIWYG) + Vault 자동 wikilink (§6.7, §11.3) |
| 자기 학습 | 없음 | **Hermes 접목** — 자기 생성 스킬 + 사용자 프로파일 + 세션 검색 + 교정 패턴 5 단계 파이프라인 (§5.5) |

### 3.2 ZeroClaw 원본 대비 MoA 만의 추가 레이어

> 원본 §★ MoA Core Workflow 의 "ZeroClaw와의 차이" 표 를 §5.4 에 전문 수록. 아래는 요약:

| 항목 | ZeroClaw (원본) | MoA (개조) |
|------|----------------|-----------|
| 채팅 방식 | CLI + 채널 | 앱채팅 GUI + 채널채팅 + 웹채팅 |
...
| 과금 | 없음 (각자 API key) | 로컬 key 무료 + 운영자 key 시 2.2× 크레딧 차감 |
| 채널 설정 | 이용자가 직접 | 운영자가 사전 설정, 이용자는 메시지만 |

### 3.3 특허화된 차별 기술 (요약)

- **Patent 1** (출원 준비) — 서버 비저장 다중 기기 E2E 암호화 메모리 동기화. §12 상세.
- **Patent 2** (출원 준비) — 에피소드 기억 × 구조적 온톨로지 간 양방향 교차 검색. §10 상세.
- **Patent 3** (준비 중, v3.0 구현 완료) — Dual-Brain compiled_truth + append-only timeline + Dream Cycle. §11 상세.
- **Patent 4** (준비 중) — MoA Vault hub note engine + 4-way RAG + 허브노트 priority queue + 3-tier conflict resolution. §11.4~§11.6 상세.
- **Patent 5 후보** (v6.1 신규) — 이용자 편집 행위 관찰 기반 자기 개선 교정 패턴. §5.5 상세.

### 3.4 정리 — "왜 MoA 인가"

- **오프라인 가능** — 로컬 SLM + 로컬 SQLite + 로컬 도구 → 네트워크 끊겨도 기본 동작.
- **비용 투명** — 이용자 key 무료, 운영자 key 2.2× 고정 (2.0× 마진 + 10% VAT), 프록시 토큰으로 key 유출 방지.
- **서버 개인정보 최소** — 기억은 기기에만 저장, 릴레이는 5 분 TTL.
- **단일 앱** — MoA + ZeroClaw 는 사용자 관점에서 하나의 설치 단위 (§4.5).
- **확장성** — 새 채널·프로바이더·도구는 전부 trait 구현 + factory 등록으로 추가.

---

## 4. 시스템 구조와 아키텍처 (Structure & Architecture)

MoA 의 런타임은 **(1) 기기별 독립 실행**, **(2) 최소 서버(Railway) 역할**, **(3) Trait-driven 모듈 확장** 세 축으로 설계되어 있습니다. 본 섹션은 원본 §2 Deployment Architecture, §6 System Architecture, §14 Technology Stack 을 한 자리에 모은 것입니다.

<!-- CHUNK: 03_deployment (원본 §2 Deployment Architecture) -->
...
**핵심 원칙:**
1. 각 MoA 앱 인스턴스는 **독립적으로 작동** — 일반 AI 동작에 서버가 필요하지 않음.
2. 각 기기는 **장기 기억을 담은 자체 SQLite** 를 보유 (임베딩은 sqlite-vec, 전문 검색은 FTS5).
3. 메모리 동기화는 **릴레이 경유 피어 투 피어** — 릴레이 서버는 데이터를 **최대 5 분** 만 보유 후 삭제.
4. 한 사용자가 MoA 를 **여러 기기에 설치** 가능 — 실시간 동기화를 통해 모두 같은 기억을 공유.
5. **일반 AI 동작은 릴레이 서버를 경유하지 않음** — 앱이 기기에서 직접 LLM API 를 호출.
6. **MoA = 단일 GUI 앱** — ZeroClaw 런타임이 sidecar 바이너리로 모든 MoA 설치 파일에 번들됨. 사용자는 파일 하나만 다운로드·설치. "ZeroClaw" 별도 설치 단계는 없음. 전체 계약은 아래 "통합 앱 경험 (Unified App Experience)" 섹션 참조.

#### LLM API Key 모델 — 3-Tier Provider Access

MoA 는 LLM 호출 라우팅·과금·모델 사용을 결정하는 **3-Tier provider access model** 을 사용합니다.
...
| Mode | 조건 | LLM 호출 경로 | 모델 선택 | 과금 |
|------|-----------|---------------|-----------------|---------|
| **UserKey** | 이용자 API key 제공 | 디바이스에서 provider 직접 호출 | 이용자가 선택 (최고사양) | 무료 (이용자가 provider 에 직접 지불) |
| **Platform (Selected)** | API key 없음 + 모델 선택됨 | Railway 릴레이 (운영자 key) | 이용자 선택 모델 | 실제 API 비용 × 2.2× 크레딧 |
| **Platform (Default)** | API key 없음 + 모델 미선택 | Railway 릴레이 (운영자 key) | 작업 유형별 자동 라우팅 | 실제 API 비용 × 2.2× 크레딧 |

##### 작업 유형별 기본 모델 라우팅 (Platform Default Mode)

이용자가 API key 도 없고 특정 모델도 선택하지 않은 경우, 시스템은 작업 유형에 가장 적합한 모델로 자동 라우팅합니다:

| 작업 카테고리 | Provider | 기본 모델 | 선정 근거 |
|---------------|----------|---------------|-----------|
...
| 시나리오 | API key 출처 | 경로 | 사용 모델 | 과금 |
|----------|---------------|-------|------------|---------|
| 이용자가 해당 provider key 보유 | 이용자 자신 | 디바이스 → Provider 직접 | 이용자 선택 (최고사양) | 무료 (이용자가 provider 에 지불) |
| 이용자 key 없음 (default) | 운영자 (Railway env) | 디바이스 → Railway 릴레이 → Provider | 작업 유형 자동 라우팅 | 실제 API 비용 × 2.2× 크레딧 |
| 이용자 key 없음 (모델 선택) | 운영자 (Railway env) | 디바이스 → Railway 릴레이 → Provider | 이용자 선택 모델 | 실제 API 비용 × 2.2× 크레딧 |
| 음성 통역 | 이용자 or 운영자 | 위와 동일 규칙 | Gemini 2.5 Flash Live API | 위와 동일 규칙 |

#### 채널을 통한 원격 접속

사용자는 메신저 채널을 통해 **어느 기기에서든** (MoA 미설치 기기 포함) 자신의 MoA 앱과 상호작용할 수 있습니다:
...
**지원 채널:**
- **KakaoTalk** (MoA 추가 — upstream ZeroClaw 에는 없음)
- Telegram
- Discord
- Slack
- LINE
...
사용자가 이 채널들을 통해 자신의 원격 MoA 기기로 메시지를 보내면, 해당 기기가 요청을 처리하고 동일한 채널로 응답을 돌려보냅니다.

#### 웹 채팅 접속 (웹채팅)

MoA 홈페이지의 웹 기반 채팅 인터페이스를 통해 다음이 가능합니다:
- 원격 MoA 앱 인스턴스에 명령 전송
- 실시간 응답 수신
- 브라우징 기기에 MoA 앱 설치 불필요
- 이용자 등록 MoA 기기와 인증 연결

#### 세 가지 채팅 방식 (3 Chat Modes)

MoA 는 서로 다른 사용자 시나리오를 위해 세 가지 AI 에이전트 상호작용 방식을 제공합니다:
...
주(primary) 채팅 경험이자 가장 풍부한 인터페이스. 사용자는 자신의 기기에 설치된 데스크톱/모바일 MoA 앱을 통해 상호작용합니다.

- **API key 해석 순서**: 로컬 key (`~/.zeroclaw/config.toml` 또는 per-provider keys) → Railway 의 운영자 key (fallback).
- **로컬 key 사용 시**: LLM 호출이 디바이스에서 provider API 로 직접 전송. Railway 미관여. 크레딧 차감 없음.
- **운영자 key 사용 시**: LLM 호출이 Railway 서버를 경유하며 운영자의 `ADMIN_*_API_KEY` 환경변수를 사용. 실제 API 비용 × 2.2× 로 크레딧 차감.
- **기능**: 풀 리치 GUI (채팅 내 markdown 렌더링, 중국·인도 방언 포함 120+ 언어 자동 감지, STT 음성 입력, TTS 음성 출력, 문서 뷰어/편집기, PDF/DOC/HTML/MD 내보내기, 파일 업로드, 모든 도구 카테고리).

##### Channel Chat (채널채팅) — 메시징 플랫폼 원격 접속

익숙한 메시징 앱으로 MoA 와 대화하려는 비기술 사용자를 위한 설계 — **사용자 측에서 별도 설정 불필요**.

- **제로 사용자 설정**: 운영자(admin) 가 모든 채널 봇 토큰·웹훅 시크릿·API key 를 Railway 환경변수로 사전 설정. 사용자는 메시징 앱에서 봇에게 메시지만 보내면 됨.
- **Railway 의 역할 (최소)**: Railway 는 웹훅 엔드포인트와 채널 설정만 호스팅. 실제 AI 처리는 Railway 에 저장된 `ADMIN_*_API_KEY` 운영자 key 를 사용.
- **지원 채널**: KakaoTalk, Telegram, Discord, Slack, LINE.
- **크레딧**: 항상 2.2× 차감 (운영자 key 사용).

###### KakaoTalk 직접 연결

KakaoTalk 은 다른 채널과 달리 독특한 아키텍처를 갖습니다:

- **웹훅 기반**: KakaoTalk 은 Kakao 서버가 등록된 웹훅 엔드포인트로 사용자 메시지를 전송하는 callback URL 패턴을 사용.
- **Railway 필요성**: KakaoTalk 이 공개 HTTPS 엔드포인트를 요구하므로, 웹훅 callback 수신을 위해 Railway (또는 공개 서버) 가 필요.
- **단, 예외**: 사용자 로컬 디바이스가 공개 IP 를 갖거나 터널(ngrok · Cloudflare Tunnel 등) 을 사용하는 경우, Kakao Developer Console 에 로컬 웹훅 URL 을 등록하면 Railway 없이 KakaoTalk 이 로컬 MoA 앱에 직접 연결 가능.
- **실용적 권장**: 대부분의 사용자에게는 로컬 터널 유지보다 Railway 호스팅이 간단하고 안정적.

###### 채널 설정 단순화 전략

최종 사용자에게 채널 접근을 최대한 단순하게 만드는 것이 목표:

| 채널 | 운영자 설정 (일회성) | 사용자 설정 | 사용자 경험 |
|---------|--------------------------|------------|-----------------|
| **KakaoTalk** | Kakao Channel 등록, Railway 에 webhook URL 설정, `KAKAO_*` env vars 추가 | KakaoTalk Channel 친구 추가 | 메시지 전송 → AI 응답 수신 |
| **Telegram** | @BotFather 로 봇 생성, Railway 에 `TELEGRAM_BOT_TOKEN` 추가 | 봇 이름 검색 후 Start 클릭 | 메시지 전송 → AI 응답 수신 |
| **Discord** | Discord App/Bot 생성, Railway 에 `DISCORD_TOKEN` 추가 | 봇 있는 서버 참여 또는 봇에게 DM | 메시지 전송 → AI 응답 수신 |
| **Slack** | Slack App 생성, Railway 에 `SLACK_*` 토큰 추가 | 워크스페이스에 앱 추가 | 메시지 전송 → AI 응답 수신 |
| **LINE** | LINE Official Account 생성, Railway 에 `LINE_*` 토큰 추가 | LINE 친구 추가 | 메시지 전송 → AI 응답 수신 |

##### Web Chat (웹채팅) — 브라우저 기반 접속

사용 중인 기기에 MoA 를 설치할 수 없는 경우(공공 PC, 도서관 컴퓨터, PC방, 빌린 기기)를 위한 방식.

- **동작 원리**: 사용자가 `mymoa.app` 에 접속해 MoA 계정으로 로그인한 후 웹 인터페이스에서 채팅.
- **경로**: 브라우저 → Railway 서버 (WebSocket) → MoA gateway → LLM.
- **API key**: 웹 설정에 입력한 자신의 key 사용 가능, 미입력 시 운영자 key 사용 (2.2× 크레딧 차감).
- **제한**: 로컬 파일 접근 불가, 로컬 도구 실행 불가 — 도구는 Railway 호스팅 gateway 인스턴스에서 실행됨.

#### 통합 앱 경험 (MoA + ZeroClaw = 하나의 앱)

> **필수 요구사항 (MANDATORY)**: 최종 사용자에게 MoA 와 ZeroClaw 는 **단일 · 분리 불가능한 애플리케이션**으로 보여야 합니다. Sidecar 아키텍처는 내부 구현 세부사항이며 사용자 경험에 절대 노출되지 않습니다.

##### 원칙

1. **하나의 다운로드, 하나의 설치, 하나의 앱** — 사용자는 설치 파일 하나(`.dmg`, `.msi`, `.AppImage`, `.apk`, `.ipa`) 만 다운로드. 이 단일 패키지에 MoA 프론트엔드(Tauri webview) 와 ZeroClaw 런타임(Rust sidecar 바이너리) 이 모두 포함됨. 별도의 "ZeroClaw installer" 는 사용자에게 노출되지 않음.
2. **제3자가 분리할 수 없음** — Sidecar 바이너리는 Tauri 의 `externalBin` 메커니즘으로 앱 패키지 내부에 번들됨. 사용자가 서비스 할 수 있는 부품이 아님. 임베드된 ZeroClaw 런타임 없이는 MoA 앱이 작동을 거부함.
3. **자동 수명주기 관리** — 앱 실행 시 MoA 가 백그라운드에서 조용히 ZeroClaw gateway 프로세스를 시작. 앱 종료 시 ZeroClaw 프로세스 종료. 크래시 발생 시 두 컴포넌트를 함께 복구. 사용자는 "ZeroClaw 시작 중…" 같은 메시지나 프로세스가 둘이라는 표시를 절대 보지 못함.
4. **통합 업데이트** — 새 버전이 나오면 Tauri 업데이터가 프론트엔드와 ZeroClaw 바이너리를 모두 포함한 업데이트 패키지 하나를 다운로드. 업데이트는 원자적 (atomic) — 두 컴포넌트가 동시 업데이트되며 비동기 상태가 되지 않음.
5. **단일 설정 플로우** — 모든 ZeroClaw 설정(API keys, 모델 선택, 채널 설정, 메모리 선호) 은 첫 실행 설정 마법사에서 MoA GUI 로 구성됨. 사용자가 수동 편집해야 하는 별도 설정 파일은 없음.
...
##### 레이턴시 계약 (Sidecar IPC 성능)

> **필수 (MANDATORY)**: Sidecar (별도 프로세스) 아키텍처는 in-process 라이브러리 임베딩 대비 **감지 가능한 레이턴시를 추가해서는 안 됩니다**.

| 통신 방식 | 왕복 레이턴시 | 상태 |
|---------------------|-------------------|--------|
| In-process (cdylib) | ~0 (nanoseconds) | 기준선 |
| Unix Domain Socket | 0.05–0.2ms | 수용 가능 |
| **WebSocket (localhost, persistent)** | **0.1–0.5ms** | **선택된 방식** |
| HTTP POST (localhost, per-request) | 1–3ms | 폴백 전용 |

**왜 수용 가능한가**: 실제 병목은 LLM API 호출(클라우드 provider 왕복 500ms–30s)입니다. 로컬 IPC 오버헤드 0.1–0.5ms 는 **전체 응답 시간의 <0.1%** 로 물리적으로 감지 불가능한 수준.

**구현 보증**:
1. MoA 는 시작 시 **persistent WebSocket** 으로 ZeroClaw 에 접속 — 메시지당 연결 셋업 오버헤드 없음.
2. 메시지는 WebSocket 위로 JSON 직렬화 — 최소 프레이밍.
3. WebSocket 연결은 `127.0.0.1` (loopback) 경유 — 네트워크 스택 미개입, 커널 메모리 복사만 발생.
4. 시간 민감 연산(음성 스트리밍, 타이핑 표시등) 에는 JSON 대신 binary WebSocket 프레임 사용.
5. End-to-end 측정: MoA 가 사용자 메시지를 전송한 시점부터 ZeroClaw 가 첫 LLM 토큰을 반환할 때까지 IPC 오버헤드는 모든 지원 플랫폼에서 **<1ms**.

**레이턴시 예산 분해 (일반 채팅 메시지)**:
```
사용자 메시지 입력   ──▸ MoA 프론트엔드 처리        ──▸  ~5ms
MoA → ZeroClaw IPC                              ──▸  ~0.3ms  ← sidecar overhead
ZeroClaw 처리 (라우팅, 메모리 recall)            ──▸  ~20ms
ZeroClaw → LLM API (네트워크 왕복)               ──▸  ~500ms–30s  ← 지배적
LLM → ZeroClaw (스트리밍 토큰)                   ──▸  연속
ZeroClaw → MoA IPC (토큰당)                      ──▸  ~0.1ms  ← sidecar overhead
MoA 프론트엔드 토큰 렌더링                       ──▸  ~1ms
───────────────────────────────────────────────────
총 sidecar overhead: 500ms+ 중 ~0.4ms = <0.1%
```

---


<!-- CHUNK: 09_system_arch (원본 §6 System Architecture) -->

### 6. 시스템 아키텍처 (System Architecture)

#### 고수준 모듈 맵 (High-Level Module Map)
...
#### 플랫폼 타겟 (Platform Targets)

| 플랫폼 | 기술 스택 | ZeroClaw 런타임 | SQLite |
|----------|-----------|-----------------|--------|
| **Windows** | Tauri 2.x | Native Rust binary | 로컬 파일 |
| **macOS** | Tauri 2.x | Native Rust binary | 로컬 파일 |
| **Linux** | Tauri 2.x | Native Rust binary | 로컬 파일 |
| **Android** | Tauri 2.x Mobile | Native Rust (NDK) | 로컬 파일 |
| **iOS** | Tauri 2.x Mobile | Native Rust (static lib) | 로컬 파일 |

모든 플랫폼은 **동일한 ZeroClaw Rust core** 를 실행합니다 — 앱은 thin client 가 아닙니다. 각 기기는 완전 자율 AI 에이전트입니다. ZeroClaw 는 MoA 앱 패키지 내부에 sidecar 바이너리 (데스크톱) 또는 static library (모바일) 로 번들됩니다. 사용자는 하나의 앱만 보고 상호작용합니다: **MoA**. ZeroClaw 런타임은 최종 사용자에게 보이지 않습니다.

#### Trait 기반 확장 지점 (Trait-Driven Extension Points)
...
| `Memory` | `src/memory/traits.rs` | 메모리 백엔드 추상화 |
| `Observer` | `src/observability/traits.rs` | Observability sink |
| `RuntimeAdapter` | `src/runtime/traits.rs` | 런타임 환경 추상화 |
| `Peripheral` | `src/peripherals/traits.rs` | 하드웨어 보드 추상화 |
| `VoiceProvider` | `src/voice/pipeline.rs` | 음성 API 스트리밍 |
| `CodeReviewer` | `src/coding/traits.rs` | AI 코드 리뷰 에이전트 |
| `OntologyRepo` | `src/ontology/repo.rs` | 구조적 관계 메모리 CRUD |

**규칙**: 새 기능은 크로스 모듈 재작성이 **아니라** 트레이트 구현 + factory 등록으로 추가합니다.

---


<!-- CHUNK: 25_tech_stack (원본 §14 Technology Stack) -->

### 14. 기술 스택 (Technology Stack)
...
| **기본 LLM** | Gemini 3.1 Flash Lite (채팅용 경제적 기본값; 다른 카테고리는 작업 기반 라우팅) |
| **음성/통역** | Gemini 2.5 Flash Native Audio (Live API) |
| **코드 리뷰** | Claude Opus 4.6 + Gemini 3.1 Pro |
| **문서 뷰어** | pdf2htmlEX (레이아웃 보존 PDF→HTML) |
| **문서 편집기** | Tiptap (ProseMirror) + tiptap-markdown bridge |
| **PDF 추출** | PyMuPDF / pymupdf4llm (구조→Markdown) |
| **문서 OCR** | Upstage Document AI (이미지 PDF 폴백) |
| **Office 변환** | Hancom API (HWP, DOCX, XLSX, PPTX) |
| **릴레이 서버** | Railway (WebSocket 릴레이, 영구 저장 없음) |
| **암호화** | AES-256-GCM (vault, sync), ChaCha20-Poly1305 (secrets), HKDF 키 유도 |
| **CI** | GitHub Actions |
...
<!-- CHUNK: 17_self_learning (원본 §6F Self-Learning Skill System) -->
...
<!-- CHUNK: 18_voice (원본 §7 Voice / Simultaneous Interpretation) -->
...
<!-- CHUNK: 19_coding_review (원본 §8 Coding / Multi-Model Review Pipeline) -->
...
<!-- CHUNK: 20_coding_sandbox (원본 §9 Coding Sandbox) -->
...
## 7. 작동 원리 (Operating Principles)

MoA 가 요청 한 건을 어떻게 계획·실행·검증·반환하는지, 그리고 어떤 설계 원칙·위험 등급 하에서 동작하는지를 정의합니다. 본 섹션은 원본 §6★ Browser Daemon & @Ref System (Plan-Execute-Verify 프로토콜 포함), §12 Design Principles, §13 Risk Tiers 를 한 자리에 모은 것입니다.

<!-- CHUNK: 10_browser_daemon (원본 §6★ Browser Daemon + gstack 스프린트 사이클 + Plan-Execute-Verify) -->

### 6★. 브라우저 데몬 & @Ref 시스템 (gstack 영감)

#### 배경

MoA 의 브라우저 자동화는 예전에 명령마다 새 프로세스를 띄웠습니다 (명령당 ~2-5 초). 20 개 명령짜리 QA 세션에는 40 초 이상의 오버헤드가 더해졌습니다. [gstack](https://github.com/garrytan/gstack) 의 브라우저 아키텍처를 도입한 후, 시스템은 **persistent Chromium daemon** 을 사용하여 서브 초 레이턴시를 달성합니다.
...
| 구성요소 | 기술 | 역할 |
|-----------|-----------|------|
| Agent Loop | Rust (browser.rs) | HTTP 로 명령 전송, 데몬 자동 기동 |
| Daemon | Node.js (`scripts/playwright-daemon.js`) | 장시간 실행 HTTP 서버, 브라우저 상태 유지 |
| Browser | Chromium (Playwright CDP) | 영속 탭·쿠키·로그인 세션 |

#### 성능

| 지표 | 이전 (per-process) | 이후 (daemon) |
|--------|---------------------|----------------|
| 첫 명령 | ~3-5초 | ~3초 (startup) |
| 이후 명령 | 명령당 ~2-5초 | **명령당 ~100-200ms** |
| 20-명령 세션 | ~60-100초 | **~7초** |
| 쿠키 지속 | 없음 (매번 초기화) | 모든 명령에 걸쳐 **지속** |
| 로그인 세션 | 매번 재인증 필요 | 브라우저 닫기 전까지 **유지** |

#### @Ref 시스템 (Accessibility Tree 참조)

깨지기 쉬운 CSS selector 대신, MoA 는 Chromium accessibility tree 로부터 유도한 안정적인 요소 참조 **@refs** 를 사용합니다:
...
**왜 @refs 가 CSS selector 를 이기는가:**

| 문제 | CSS/XPath | @Ref 시스템 |
|---------|-----------|-------------|
| Content Security Policy | DOM 주입 차단됨 | DOM 변형 불필요 |
| React/Vue hydration | 주입 속성이 제거됨 | 외부 accessibility tree |
| Shadow DOM | 내부 접근 불가 | Chromium 내부 트리 |
| DOM 구조 변경 | selector 깨짐 | role 기반, 구조 독립적 |

**Stale 감지 (Staleness detection):** SPA 가 DOM 을 변형하면 ref 가 stale 해질 수 있습니다. 어떤 ref 를 사용하기 전에 시스템은 async `count()` 체크(~5ms) 를 수행합니다. 요소가 사라지면 *"@e3 is stale. Run snapshot to get fresh refs."* 안내와 함께 실패합니다. ref 는 페이지 이동 시 자동 초기화됩니다.

#### 데몬 수명주기 (Daemon Lifecycle)

```
앱 시작 → 첫 브라우저 명령 → 포트 9500 에서 데몬 자동 기동
                                     ↓
                              Chromium 기동 (headless)
                                     ↓
                              상태 파일 기록:
...
**크래시 복구:** Chromium 이 크래시되면 데몬이 즉시 종료됩니다. 다음 명령이 자동 재시작합니다 — 재연결보다 단순하고 안정적.

#### 명령 카테고리 (Command Categories)

| 카테고리 | 명령 | 특성 |
|----------|----------|-----------------|
| **Navigate** | open, back, forward, reload | 페이지 이동 |
| **Snapshot** | snapshot | accessibility tree 로부터 @ref 맵 구축 |
| **Interact** | click, fill, type, press, hover, scroll, select | 페이지 상태 변형 |
| **Read** | text, html, links, forms, cookies, url | 데이터 추출, 변형 없음 |
| **Visual** | screenshot | PNG 캡처 (전체 페이지 · 뷰포트 · 요소) |
| **Tabs** | tabs, newtab, tab, closetab | 멀티 페이지 워크플로 |
| **Script** | js, eval | JavaScript 실행 |
| **Lifecycle** | close | 브라우저 종료 |

#### 작업 카테고리 통합

모든 MoA 작업 카테고리가 persistent browser 의 혜택을 받습니다:
...
MoA 개발은 gstack 의 구조화된 워크플로를 따릅니다:
...
| 단계 | 도구 | 역할 |
|-------|------|-------------|
| Plan | `/autoplan` | CEO → Design → Eng 리뷰 자동 수행 |
| Review | `/review` | Staff 레벨 코드 리뷰, 자동 수정 |
| Test | `/qa` | 실제 Chromium 브라우저 테스트 + 회귀 테스트 |
| Security | `/cso` | OWASP Top 10 + STRIDE 위협 모델링 |
| Ship | `/ship` | main 동기화, 테스트, PR |
| Reflect | `/retro` | 주간 회고 |

#### Plan-Execute-Verify 프로토콜

모든 사용자 요청은 구조화된 4-phase 프로토콜을 따릅니다:

1. **Phase 1 — 분석 & 계획**: 요청 분류, 가용 도구 스캔, 최적 도구 선택, 단계별 실행 계획 설계, 성공 기준 설정, `task_plan` 도구로 계획 등록.
2. **Phase 2 — 실행**: 선택한 도구로 계획을 단계별 실행. 웹 검색은 **Playwright 브라우저 검색이 기본값** (아래 Web Research Architecture 참조). API 기반 검색(DuckDuckGo, Jina) 은 폴백.
3. **Phase 3 — 검증**: Self-check 루프 (최대 2 회 재시도) — 완전성 · 정확성 · 최신성 · 충분성 체크. 부족 시 Phase 2 로 돌아가 키워드 정제.
4. **Phase 4 — 제시**: 직접 답변 먼저 → 근거 상세 → 출처 URL → 2-3 개 후속 제안. 사용 언어에 맞춘 포맷.

#### 웹 리서치 아키텍처 (Playwright-First)

MoA 는 전통적인 API 기반 검색 대신 **Playwright 브라우저 우선(Playwright browser-first)** 접근을 사용합니다. Persistent Chromium 데몬이 봇 탐지 문제를 제거하고 병렬 멀티 엔진 검색을 가능케 합니다.
...
| 우선순위 | Provider | 방식 | 속도 | 비용 | 봇 탐지 |
|----------|----------|--------|-------|------|---------------|
| **1 (Default)** | `browser` | Playwright: Naver+Google+DDG 병렬 | ~2초 | 무료 | 없음 |
| 2 (Fallback) | `duckduckgo` | HTTP API (HTML 스크래핑) | ~1초 | 무료 | 가능성 있음 |
| 3 (Fallback) | `jina` | Jina Search API | ~1초 | Free tier | 없음 |
| Optional | `brave`, `perplexity`, `exa` | API | ~1초 | Paid | 없음 |

##### Depth vs Breadth 탐색 규칙
...
- **왜 Playwright 가 기본인가?** DuckDuckGo HTTP API 는 User-Agent 기반 봇 탐지로 인해 빈번하게 차단됨. Playwright 는 실제 Chromium 을 사용하므로 차단이 불가능하고, Naver 검색은 한국어 쿼리에서 가장 정확한 결과를 제공.
- **왜 병렬 3-사이트인가?** 단일 사이트 검색과 동일한 ~2 초 안에 3 배의 결과를 얻을 수 있음. 각 검색엔진의 강점(Naver: 한국어, Google: 영어/범용, DDG: 프라이버시) 을 동시에 활용.
- **왜 검색과 스크래핑이 한 단계인가?** 기존 방식(web_search → web_fetch 2단계) 은 ~4 초 소요. Playwright 는 검색 페이지를 열면서 동시에 텍스트를 추출하므로 ~2 초로 단축.

#### 암호화 자격증명 볼트 & 브라우저 자동화
...
##### 참조 토큰 플로우 (Reference Token Flow)

LLM 은 실제 비밀번호를 절대 알 수 없습니다:
...
| Tool | 용도 | 기본값 | 비용 |
|------|---------|---------|------|
| `web_search` | 웹 검색 (Playwright 병렬 기본) | Enabled | 무료 |
| `web_fetch` | URL 텍스트 추출 (HTML→Markdown) | Enabled | 무료 |
| `http_request` | 범용 HTTP 요청 | Enabled (allowlist 필요) | 무료 |
| `browser` | Chromium 자동화 (@ref 시스템) | Enabled | 무료 |
...
<!-- CHUNK: 23_design_principles (원본 §12 Design Principles) -->

### 12. 설계 원칙 (Design Principles)

이들은 **필수 제약**이며 가이드라인이 아닙니다:

| 원칙 | 규칙 |
|-----------|------|
| **KISS** | 영리한 메타 프로그래밍 대신 직관적 제어 흐름 선호 |
| **YAGNI** | 추측성 기능 금지 — 구체적 · 수용된 사용 사례 필수 |
| **DRY + Rule of Three** | 3 회 이상 반복 시에만 공유 로직 추출 |
| **SRP + ISP** | 모듈당 한 가지 관심사, 좁은 트레이트 인터페이스 |
| **Fail Fast** | 미지원 상태는 명시적 오류, 절대 조용히 확장하지 않음 |
| **Secure by Default** | Deny-by-default, 시크릿 로깅 금지, 노출 최소화 |
| **Determinism** | 재현 가능한 동작, flaky 테스트 금지 |
| **Reversibility** | 작은 커밋, 명확한 롤백 경로 |

---


<!-- CHUNK: 24_risk_tiers (원본 §13 Risk Tiers) -->

### 13. 리스크 등급 (Risk Tiers)

| 등급 | 범위 | 리뷰 깊이 |
|------|-------|--------------|
| **Low** | docs, chore, tests-only | 경량 체크 |
| **Medium** | 대부분의 `src/**` 동작 변경 | 표준 리뷰 |
| **High** | `src/security/**`, `src/runtime/**`, `src/gateway/**`, `src/tools/**`, `.github/workflows/**`, `src/sync/**`, `src/ontology/**` | 전체 검증 + 경계 테스트 |
...
| 도구 이름 | 카테고리 | `safe_for_slm` | 상세 섹션 |
|---|---|---|---|
| `memory_store` / `memory_recall` | Memory | ✅ | §10 |
| `memory_timeline_append` / `memory_set_truth` | Memory (Dual-Brain) | ✅ | §11 |
| `smart_search` | Web (free → Perplexity cascade, 4 회 재조합) | ✅ | §5 (Phase 2) |
| `web_search` / `perplexity_search` | Web (단독) | ✅ | §5 / §8.2 |
| `browser` | Web (Playwright daemon, @ref system) | ✅ | §7 (Browser Daemon) |
| `file_read` | 파일 I/O (읽기) | ✅ | §8.2 |
| `file_edit` / `file_write` / `apply_patch` | 파일 I/O (쓰기) | ❌ — cloud LLM only | §5.3 (safe_for_slm) |
| `shell` | 시스템 명령 실행 | ❌ — cloud LLM only | §5.3 |
| `delegate` | 하위 에이전트 위임 | ❌ — cloud LLM only | §5.3 |
| `cron_*` | 스케줄러 관리 | ❌ — cloud LLM only | §5.3 |
| `image_generate` (Freepik Mystic) | Media | ✅ | §6.2 Media Gen |
| `image_upscale` (Freepik Magnific) | Media | ✅ | §6.2 |
| `image_to_video` (Freepik) | Media | ✅ | §6.2 |
| `video_generate` (Runway Gen-4) | Media | ✅ | §6.2 |
| `music_generate` (Suno) | Media | ✅ | §6.2 |
| `elevenlabs_tts` (ElevenLabs) | Media (Dual billing) | ✅ | §6.2 |
| `calendar_list_events` / `calendar_create_event` | Calendar (Google / Outlook / 카톡캘린더) | ✅ | §6.2 |
| `task_plan` / `task_list` | 에이전트 내부 계획 관리 | ✅ | §7 Plan-Execute-Verify |
| `skill_view` / `skill_manage` | 자기 학습 스킬 시각화·관리 | ✅ | §5.5 Self-Learning |
| `session_search_tool` | 과거 세션 FTS5 검색 | ✅ | §5.5 |
| `correction_recommend` | 교정 패턴 추천 | ✅ | §5.5 |

### 8.2 도구 실행 경로

- **Agent Loop (`src/agent/loop_.rs::process_message_with_session`)** — LLM 이 tool_use 를 emit → dispatcher 가 등록된 도구 실행 → `ToolResult` 반환 → LLM 피드백 루프.
- **SLM Executor (`src/advisor/slm_executor.rs::SlmExecutor::run`)** — Gemma 4 가 XML 프로토콜로 tool_call 을 내면 → dispatcher → `safe_for_slm=true` 인 도구만 실행 → user turn 에 `<tool_result>` 로 feedback (§5 Phase 3 참조).
- **Browser daemon** — 모든 `browser(action=...)` 호출은 `scripts/playwright-daemon.js` (Node.js) 의 HTTP 서버를 통해 persistent Chromium 에 전달 (§7 Browser Daemon 상세).
- **Media Gen API** — `src/tools/media_gen.rs` 내 각 도구가 Freepik/Runway/Suno/ElevenLabs REST API 호출. dual-billing (user-key vs operator-key) 은 `src/billing/llm_router.rs::AdminKeys` 가 처리 (§6.2 상세).

### 8.3 새로운 도구 추가 절차

1. `Tool` trait (`src/tools/traits.rs`) 구현, `safe_for_slm()` 명시적 선언 (기본 `true`).
2. `src/tools/mod.rs` 의 factory 에 등록.
3. 파라미터 스키마·검증 로직은 도구 파일에 co-located.
4. 위험 도구(파일 쓰기·shell·delegate 계열) 는 반드시 `safe_for_slm = false` override.
5. 필요한 경우 `src/config/schema.rs` 에 per-tool 설정 추가.
6. 단위 테스트 + (가능 시) integration 테스트 추가.

> **원칙 참조**: §7.2 Design Principles 의 "Trait 기반 확장" 과 "Secure by Default" 를 준수.
...
| 방식 | 경로 | 주 사용자 | API key | 상세 설계 |
|---|---|---|---|---|
| **① 앱채팅** | 로컬 Tauri 앱 → 로컬 gateway (`127.0.0.1`) | MoA 설치 사용자 | 로컬 key 우선 → 프록시 폴백 | §5 / §4.5 Sidecar |
| **② 웹채팅** | 브라우저(mymoa.app) → Railway `/ws/chat` → 디바이스 릴레이 | MoA 미설치 원격 PC | 디바이스 릴레이 시 로컬 key / 폴백 시 운영자 key | §5 / §9.2 아래 |
| **③ 채널채팅** | KakaoTalk·WhatsApp·텔레그램·디스코드·... → Railway 웹훅 → 디바이스 릴레이 | 메신저 사용자 (설치 불필요) | 디바이스 릴레이 또는 운영자 key 2.2× | §5 / §9.3 아래 |

### 9.2 Web Chat — mymoa.app 원격 접속

<!-- CHUNK: 13_web_chat (원본 §6B Web Chat & Homepage Integration) -->

### 6B. 웹 채팅 & 홈페이지 통합 아키텍처

#### 개요

MoA 는 네이티브 Tauri 앱 외에 두 개의 웹 기반 프론트엔드를 제공합니다:

1. **웹 대시보드** (`web/`) — 에이전트 채팅·설정·비용 모니터링·크론 작업·디바이스 관리 등을 위한 풀 기능 관리 UI.
2. **메인 웹사이트 / 홈페이지** (`site/`) — 제품 안내·요금제·인증 사용자 대상 웹 채팅 진입점을 제공하는 공개 랜딩 페이지.

두 프론트엔드 모두 Vite + React + TypeScript 애플리케이션으로 독립적으로 서빙됩니다. 실시간 통신은 사용자의 MoA gateway 에 WebSocket 으로 연결하여 수행합니다.
...
├── src/
│   ├── pages/
│   │   ├── AgentChat.tsx      # 주 채팅 인터페이스:
│   │   │                      #   - Markdown 렌더링 (marked 라이브러리)
│   │   │                      #   - 120+ 언어 자동 감지 (Unicode + 휴리스틱)
│   │   │                      #   - 언어 선호 persist (memory + localStorage)
│   │   │                      #   - STT 음성 입력 (Web Speech API, 크로스브라우저)
│   │   │                      #   - TTS 음성 출력 (speechSynthesis, 음성 자동 선택)
│   │   │                      #   - DOC/MD/TXT 내보내기
│   │   │                      #   - 언어 표시 포함 Voice mode
│   │   │                      #   - 연결 상태 표시
│   │   ├── Config.tsx         # 에이전트 설정
│   │   ├── Cost.tsx           # 사용량 · 과금 대시보드
│   │   ├── Cron.tsx           # 예약 작업
│   │   ├── Dashboard.tsx      # 개요 / 홈
│   │   ├── Devices.tsx        # 멀티 디바이스 관리 & 동기화 상태
│   │   └── ...
│   ├── components/            # 공용 React 컴포넌트
│   ├── lib/
│   │   ├── api.ts             # Bearer 토큰 인증 API 클라이언트
│   │   ├── auth.ts            # 토큰 관리 (session/localStorage)
│   │   └── ws.ts              # 세션 관리 WebSocket 클라이언트
│   └── App.tsx                # 라우트 정의
├── dist/                      # 빌드된 프론트엔드 산출물 (rust-embed 용 git 추적)
│   ├── index.html             # CSP 헤더 포함 SPA 진입점
│   └── assets/                # Vite 번들 JS/CSS (content hash)
├── vite.config.ts             # base: "/_app/", localhost:8080 프록시
└── package.json               # Build: tsc -b && vite build
```

##### 프론트엔드 빌드 파이프라인

웹 프론트엔드는 컴파일 타임에 `rust-embed` 를 통해 ZeroClaw Rust 바이너리에 임베드됩니다. 두 Dockerfile 모두 `node:22-alpine` 웹 빌더 스테이지를 포함하여 `npm ci && npm run build` 를 자동 실행하므로, 프로덕션 빌드에서 프론트엔드 산출물이 항상 최신 상태로 유지됩니다. `web/dist/` 의 빌드 산출물은 git 에도 추적되므로 (일반 `dist/` gitignore 규칙에서 제외), Node.js 없이도 로컬 `cargo build` 에서 그대로 사용됩니다.
...
├── src/
│   ├── pages/
│   │   ├── Landing.tsx        # 제품 개요 홈페이지
│   │   ├── Pricing.tsx        # 크레딧 패키지 & API key 모델
│   │   ├── WebChat.tsx        # 인증 사용자 대상 웹 채팅 위젯
│   │   └── ...
│   ├── components/
│   └── App.tsx
├── vite.config.ts
...
#### Gateway WebSocket 엔드포인트 (`src/gateway/`)

ZeroClaw gateway (Axum HTTP/WebSocket 서버) 가 노출하는 엔드포인트 — Tauri 앱과 웹 프론트엔드가 공통으로 접속합니다:

| 엔드포인트 | 모듈 | 용도 |
|----------|--------|---------|
| `/ws/chat` | `src/gateway/ws.rs` | 실시간 채팅 스트리밍 (텍스트 메시지, 도구 결과) |
| `/ws/voice` | `src/gateway/ws.rs` | 음성 통역 오디오 스트리밍 |
| `/api/*` | `src/gateway/api.rs` | 설정 · 메모리 · 디바이스 관리용 REST API |
| `/remote/*` | `src/gateway/remote.rs` | 크로스 디바이스 채널 라우팅 원격 접속 릴레이 |
...
브라우저 (site/ 또는 web/)
    │
    │  /ws/chat 에 WebSocket 접속
    │  (디바이스 토큰으로 인증)
    ▼
Gateway (src/gateway/ws.rs)
    │
    │  Agent 오케스트레이션 루프로 라우팅
    ▼
Agent (src/agent/loop_.rs)
    │
    ├── 메모리에서 recall (SQLite + 온톨로지 컨텍스트)
    ├── LLM 프로바이더 호출
    ├── 필요 시 도구 실행
    └── WebSocket 으로 응답 토큰 스트리밍
    │
    ▼
브라우저가 스트리밍 응답을 렌더링
```

사용자는 네이티브 앱 설치 없이 홈페이지에서 자신의 MoA 에이전트와 대화할 수 있습니다 — gateway 는 인증된 브라우저 세션의 WebSocket 연결을 처리합니다. 메모리·온톨로지 상태·동기화는 클라이언트가 Tauri 앱이든 웹 브라우저든 관계없이 **동일하게 작동**합니다.

**주 사용 시나리오**: 공공 PC·도서관 컴퓨터·PC방 등 사용자가 MoA 를 설치할 수 없는 기기. 사용자는 `mymoa.app` 에 접속해 계정으로 로그인한 후 웹 인터페이스로 채팅합니다. 웹 채팅은 Railway 호스팅 gateway 인스턴스에 WebSocket 으로 연결됩니다.
...
<!-- CHUNK: 12_digital_twin (원본 §6A Structured Relational Memory — Digital Twin Graph Layer) -->
...
<!-- CHUNK: 14_document_editor (원본 §6C Document Processing & 2-Layer Editor Architecture) -->
...
<!-- CHUNK: 15_vault (원본 §6D MoA Vault — Second Brain v6) -->
...
#### 6D-14. Domain DB 통합 — Swappable 도메인 코퍼스 (v8 추가)

**문제 정의**. v6/v7 세컨드브레인은 사용자 노트(개인 지식)와 도메인 코퍼스(법령·판례 등 미리 빌드된 참조 지식)를 모두 단일 `brain.db`에 저장했다. 이로 인해:

1. **재배포 비용**: 도메인 코퍼스(GB 단위 법령/판례)를 갱신할 때마다 사용자의 brain.db 전체를 다시 받아야 했다.
2. **개인정보 결합**: 도메인 corpus의 무결성을 보장하면서 사용자 노트만 백업/동기화/리셋하기 어렵다.
3. **확장 봉쇄**: 다른 도메인(의료·생물·금융 등)을 추가하려면 사용자의 brain.db에 영구적으로 합쳐야 했다.

**v8 해법**. `<workspace>/memory/` 안에 두 개의 SQLite 파일을 두고, 동일 Connection에 ATTACH로 결합한다:

```
<workspace>/memory/
├── brain.db    ← 사용자의 개인 vault (notes, episodes, ontology; 항상 존재)
└── domain.db   ← 교체 가능한 도메인 코퍼스 (법령·판례 등; 선택사항)
```

| 항목 | 설계 |
|---|---|
| **스키마 통일** | 두 파일 모두 동일 `vault::schema::init_schema`로 초기화 — `vault_documents`, `vault_links`, `vault_aliases`, `vault_frontmatter`, `vault_tags`, `vault_docs_fts` 동일 셋. |
| **결합 방식** | `ATTACH DATABASE '<path>/domain.db' AS domain;` — 단일 Connection이 `main.vault_documents`(brain.db)와 `domain.vault_documents`(domain.db)를 모두 본다. 트랜잭션·WAL·잠금 모두 SQLite가 자동 처리. |
| **Swap 흐름** | (1) `detach_domain()` → (2) staging 복사 + atomic `rename()` → (3) `attach_domain()`. 진행 중 크래시는 항상 "옛 파일 또는 새 파일" 둘 중 하나를 남긴다. |
| **사용자 데이터 보호** | 도메인 swap은 `brain.db`를 단 한 byte도 건드리지 않는다. 사용자의 개인 노트·위키링크·온톨로지 모두 도메인 업그레이드 후에도 그대로 유지. |

**모듈 구성**:

| 파일 | 역할 |
|---|---|
| `src/vault/domain.rs` | Lifecycle (`ensure_schema`, `attach`, `detach`, `is_attached`, `install_from`, `uninstall`, `info`) — 단일 책임의 helper. **[구현]** 10 테스트 |
| `src/vault/store.rs` | `VaultStore::open_for_workspace(conn, workspace_dir)` — 자동 ATTACH 팩토리. `attach_domain` / `detach_domain` / `has_domain` 노출. **[구현]** 3 테스트 |
| `src/vault/legal/ingest.rs` | `IngestTarget::{Main, Domain}` enum + `ingest_statute_to`/`ingest_case_to`/`resolve_pending_links_in` 변형. 모든 SQL이 `{schema}.vault_*` 형태로 fully-qualified. 기존 `ingest_statute`/`ingest_case`는 `Main` 라우팅 shim으로 보존. **[구현]** |
| `src/vault/legal/graph_query.rs` | 모든 read 사이트(`get_node`, `read_article`, `find_nodes`, `fetch_direct_neighbors`, `fetch_edges_within`, `pick_applicable_version`, `list_supplements_of_law`, `load_frontmatter`)가 활성 schema 리스트(domain → main)를 순회하며 결과를 병합. **[구현]** |

##### 6D-14.1 Slug 충돌 정책 — Domain Priority

동일 slug(예: `statute::근로기준법::36`)가 `main`과 `domain`에 모두 존재할 때, **domain 행이 main 행을 가린다**.

```
SCHEMAS_BY_PRIORITY = ["domain", "main"]   // src/vault/legal/graph_query.rs
```

읽기 함수는 위 순서로 schema를 순회해 첫 번째 hit를 채택한다. 이유:

- 도메인 코퍼스는 큐레이션·법령정보API 검증을 거친 **권위 있는 source of truth**.
- 사용자가 같은 slug로 무엇을 적었더라도 도메인 데이터를 절대 덮어쓰지 못해야 한다 — 법조문 무결성 invariant.
- 사용자 슬러그 충돌은 노이즈로 간주, 도메인 데이터로 응답하면서도 `lookup_slug_in_legal`이 schema 출처를 함께 반환하므로 클라이언트는 추후 두 행을 모두 표시할 수 있다.

##### 6D-14.2 사용자 노트 → 도메인 wikilink 해결 — Slug-only Resolution

사용자가 자기 노트에 `[[근로기준법 제36조]]`를 작성한 경우의 흐름:

```
brain.db.vault_links
┌──────────────┬──────────────────────────┬──────────────┬─────────────┐
│ source_doc   │ target_raw               │ target_doc_id│ is_resolved │
├──────────────┼──────────────────────────┼──────────────┼─────────────┤
│ 42 (내 노트) │ "statute::근로기준법::36"│ NULL         │ 0           │
└──────────────┴──────────────────────────┴──────────────┴─────────────┘
```

- **target_doc_id를 의도적으로 비워둔다 (NULL)**. 도메인 swap 시 ID가 달라져도 링크가 깨지지 않도록 cross-schema integer FK를 절대 캐시하지 않는다.
- 조회 시 `fetch_direct_neighbors` 등이 모든 활성 schema의 `vault_documents`에 대해 slug(`title`)로 lookup해 동적으로 해결.
- 인제스트 시점에는 같은 schema 내 target만 `target_doc_id`를 채운다 (intra-schema fast path; 도메인 corpus 내 statute→statute 인용 등).

이 정책으로 얻는 이점:

| 이점 | 메커니즘 |
|---|---|
| **Swap-safe** | domain.db 새 버전을 받아 갈아끼워도 main.vault_links가 손상되지 않음 — 캐시된 ID가 없으므로. |
| **테이블 추가 없음** | 별도 cross_links 테이블 불필요. KISS/YAGNI 원칙 준수. |
| **성능 적정** | slug에 인덱스가 있어 ATTACH UNION lookup은 ms 단위. 도메인이 미부착이면 자동으로 `main` 단일 schema로 fallback. |

##### 6D-14.3 SQLite FTS5 제약 — MATCH는 schema-qualified 불가

발견된 제약: SQLite FTS5의 `MATCH` 연산자 LHS는 **unqualified FTS5 table 이름**만 받는다. `main.vault_docs_fts MATCH ?` / `domain.vault_docs_fts MATCH ?` / 별칭 `f MATCH ?` 모두 `no such column` 오류로 거부된다.

**대응 정책**: `find_nodes`의 5단계 FTS 폴백은 **`main.vault_docs_fts`만** 조회한다. 도메인 코퍼스에 대한 본문 검색은 의도적으로 entry point 밖으로 옮기고, 별도 connection을 여는 도메인 전용 도구(예: 향후 `domain_fts_search`)로 처리. **slug + alias lookup(2~4단계)은 cross-schema 동작**하므로 `legal_graph_find` 등 일반 호출은 도메인을 정상적으로 조회.

##### 6D-14.4 Read-time UNION 패턴

`graph_query.rs` read 사이트가 모두 동일한 패턴을 사용:

```rust
fn active_schemas(conn: &Connection) -> &'static [&'static str] {
    if domain_attached(conn) {
        &["domain", "main"]   // domain priority
    } else {
        &["main"]             // single-DB fallback
    }
}
```

ID 기반 종속 쿼리(`load_frontmatter`, content 조회 등)는 slug lookup 단계에서 얻은 schema 이름을 항상 함께 전달해 **scope locality**를 유지 — SQLite ID는 schema-scoped이므로 cross-schema ID 사용은 미정의 동작이다.

##### 6D-14.5 도메인 라이프사이클 (CLI / Tools)

| 단계 | API | 상태 |
|---|---|---|
| **A1+A2** Lifecycle helpers | `domain::{ensure_schema, attach, detach, install_from, uninstall, info}` + `VaultStore::open_for_workspace` | **[구현]** |
| **A3+A4** Ingest routing + read UNION | `ingest_statute_to(Domain)`, graph_query 8개 사이트 cross-schema, `vault legal ingest` CLI가 domain.db로 라우팅 | **[구현]** |
| **A5** Migration helper | `vault domain extract` — brain.db에 잘못 들어간 legal rows를 domain.db로 이관 | **[계획]** |
| **B** CLI surface | `vault domain {install,update,swap,uninstall}` + R2 manifest downloader | **[계획]** |
| **C** Pre-bake pipeline | `vault domain build` + R2 publish 스크립트 (도메인 코퍼스 빌드/배포 자동화) | **[계획]** |

##### 6D-14.6 특허 청구항 (추가)

- **청구항 30**: 청구항 28에 있어서, 세컨드브레인의 저장 계층은 사용자의 개인 vault DB와 교체 가능한 도메인 코퍼스 DB로 **물리적으로 분리**되어 동일 SQLite Connection에 ATTACH 방식으로 결합되며, 도메인 코퍼스 DB의 교체(swap)는 사용자 vault DB의 어떤 행도 변경하지 않으면서 후속 통합 검색 호출에서 **자동으로 새 코퍼스를 우선 매칭**시키는 것을 특징으로 하는 시스템.
- **청구항 31**: 청구항 30에 있어서, 사용자 vault DB에 저장된 위키링크 행 중 도메인 코퍼스 DB의 노드를 가리키는 cross-schema 참조에 대해, 정수 외래 키를 영속화하지 않고 **slug(논리 식별자)만을 저장**한 뒤 조회 시점에 활성 schema 우선순위에 따라 동적으로 해결함으로써 도메인 코퍼스 DB 교체 후에도 위키링크 무결성을 보장하는 것을 특징으로 하는 시스템.

#### 6D-13. 모듈 파일 인덱스 — v8 갱신

(위 §6D-13 표 + 다음 행 추가)

| 파일 | 역할 | 상태 |
|---|---|---|
| `src/vault/domain.rs` | Domain DB lifecycle (attach/detach/install/uninstall/info) | **[구현]** |
| `src/vault/legal/ingest.rs` (확장) | `IngestTarget` enum + 도메인 schema 라우팅 | **[구현]** |
| `src/vault/legal/graph_query.rs` (확장) | Cross-schema UNION reads + domain priority resolution | **[구현]** |
| `src/vault/domain_migrate.rs` | Step A5 — brain.db→domain.db 마이그레이션 헬퍼 | **[구현]** |
| `src/vault/domain_manifest.rs` | Step B2 — manifest schema + sha256 검증 다운로더 | **[구현]** |
| `src/vault/domain_cli.rs` | Step B1+C — `vault domain {info,extract,install,update,swap,uninstall,build,publish}` | **[구현]** |

#### 6D-15. Korean legal corpus — 6-category recognition (v8.1)

`vault domain build` 가 사용자 코퍼스(예: `C:\Users\<user>\세컨드브레인_로컬DB\`) 하부에서 다음 6개 디렉토리 카테고리를 자동 인식한다 (`src/vault/legal/source_path.rs::detect_category`).

| 디렉토리 | `SourceCategory` | 분류 | 쓰기 정책 |
|---|---|---|---|
| `현행법령/<YYYYMMDD>/...` | `Current` | 법령 | canonical + versioned 슬러그 |
| `연혁법령/<YYYYMMDD>/...` | `Historical` | 법령 | versioned only (canonical 보존) |
| `자치법규/...` | `LocalOrdinance` | 법령 | canonical only |
| `행정규칙/...` | `AdminRule` | 법령 | canonical only |
| `판례/<YYYYMMDD>/...` | `Case` | 판례 | 사건번호 슬러그 1개 |
| `헌재결정례/<YYYYMMDD>/...` | `ConstitutionalCase` | 판례 | 사건번호 슬러그 1개 |

`is_statute_kind()` / `is_case_kind()` 헬퍼는 routing 결정에 사용. content 휴리스틱(`looks_like_case`/`looks_like_statute`)이 path 카테고리와 충돌할 때 path가 case-shaped이면 case로 우선 라우팅 (mis-filed 보정).

**`헌재결정례` 본문 호환성** — `결정요지` / `결정문` 섹션을 `판결요지` / `판례내용`과 동일하게 인식 (`case_extractor.rs::extract_case`).

##### 6D-15.1 핵심 키워드 자동 인덱싱 (v8.1)

사용자 명시 우선순위: 판례의 **적용법조(적용법령/적용법률)** + 판례의 **사건명** + 법령 조항의 **소제목 명사** = 세컨드브레인 검색의 1순위 키워드.

| 출처 | 추출 결과 | 등록처 |
|---|---|---|
| 판례 `## 적용법조` / `적용법령` / `적용법률` | `cites` 엣지 (statute slug 타깃) | `vault_links` |
| 판례 `## 참조조문` (보조) | `cites` 엣지 (dedup) | `vault_links` |
| 판례 `## 판례내용` / `결정문` (본문) | `cites` 엣지 (dedup) | `vault_links` |
| 판례 사건명 (`## 사건명`) | bare alias + `<사건번호> <사건명>` alias + `title:<사건명>` 태그 | `vault_aliases`, `vault_tags` |
| 법령 조항 소제목 (괄호 내 명사구) | bare alias + `<법령명> <소제목>` alias + `<소제목>` 태그 (`title_kw` 타입) | `vault_aliases`, `vault_tags` |

이 alias 등록 덕분에 `find_nodes` 의 2단계(alias) 매칭에서 사용자 노트의 `[[금품 청산]]` 또는 `[[근로기준법위반]]` 같은 자연어 위키링크가 도메인 노드로 즉시 resolve 된다 — FTS5 단계까지 내려가지 않고도.

##### 6D-15.2 빌드 리포트 — corpus mix

`vault domain build` 종료 시 다음 진단 출력:
...
`unknown` 카테고리에 잡힌 파일은 6개 마커 어느 것도 path에 없는 케이스이므로, 폴더 구조를 점검하라는 시그널.

---


<!-- CHUNK: 16_traceability (원본 §6E Dual-Brain 구현 검증 매트릭스 — Plan ↔ Code Traceability) -->
...
## 12. 복수 디바이스 실시간 동기화 (Multi-Device Sync)

복수 디바이스 간 기억·대화·설정·스킬을 **서버 비저장 방식으로 E2E 암호화 동기화** 하는 특허 1 의 구현체입니다. Layer 1 (5 분 TTL 릴레이) + Layer 2 (델타 저널 + 버전 벡터 + 순서 버퍼) + Layer 3 (manifest 기반 full sync) 의 3-tier 계층 구조로 구성되며, v3.0 Sync Journal 에서 Dual-Brain (timeline / compiled_truth / phone_calls) 과 자기 학습 스킬 (procedural / user_profile / correction_pattern) 델타까지 통합되었습니다. 본 섹션은 원본 §3 Patent E2E Sync 와 §3c Sync Journal v3 을 한 자리에 모은 것입니다.

<!-- CHUNK: 04_patent_sync (원본 §3 Patent: Server-Non-Storage E2E Encrypted Memory Sync + Patent 2 청구항) -->
...
<!-- CHUNK: 06_sync_journal (원본 §3c Sync Journal v3 — Dual-Brain 동기화 통합) -->
...
## 13. 부록 — Config · Patents · Roadmap · 구현 감사 · For AI Reviewers

본 섹션은 원본 §10 Configuration Reference, §11 Patent-Relevant Innovation Areas, §15 Implementation Roadmap, §15A Implementation Details Beyond Original Spec, §16 For AI Reviewers 를 한 자리에 모은 부록입니다.

### 13.1 설정 레퍼런스 (Configuration Reference)

<!-- CHUNK: 21_config (원본 §10 Configuration Reference) -->
...
### 13.2 Patent-Relevant Innovation Areas

<!-- CHUNK: 22_patent_areas (원본 §11 Patent-Relevant Innovation Areas) -->

### 11. 특허 관련 혁신 영역 (Patent-Relevant Innovation Areas)

#### Innovation 1: 서버 비저장 E2E 암호화 메모리 동기화

전체 명세는 [§3 (Patent: Server-Non-Storage E2E Encrypted Memory Sync)](#3-patent-server-non-storage-e2e-encrypted-memory-sync) 참조.

**청구 요지**: 델타 기반 동기화, 5 분 TTL 릴레이, zero-knowledge 서버, 디바이스 로컬 authoritative 저장, 오프라인 reconciliation.

#### Innovation 2: 동시 통역을 위한 Commit-Point 세그멘테이션

3-포인터 아키텍처(committed | stable-uncommitted | unstable) + 하이브리드 경계 감지(구두점·무음·길이 상한) 를 사용한 실시간 구 단위 오디오 번역. 화자가 **문장을 끝내기 전**에 번역을 시작할 수 있음.

#### Innovation 3: 멀티 모델 합의(consensus) 코드 리뷰 파이프라인

Model A (Claude) 가 코드를 생성 → Model B (Gemini) 가 아키텍처 정합성을 리뷰 → Model A 가 Model B 의 findings 를 검증 → 파이프라인이 severity 가중 중복 제거로 findings 병합. AI 모델이 **자율적으로 코드 품질을 토론·개선**.

#### Innovation 4: 작업 카테고리 인지 도구 라우팅

작업 카테고리별로 도구 가용성을 동적으로 조정 — 각 카테고리는 자신의 도메인 관련 도구만 노출. 공격 표면 축소 + 모델 집중도 향상. Coding 카테고리는 모든 도구, Translation 카테고리는 최소 도구.

#### Innovation 5: 자율 수리 루프 포함 6-Phase 구조화 코딩

Comprehend → Plan → Prepare → Implement (run→observe→fix) → Validate → Deliver. 오류 분류, 반복 오류 감지, 롤백 체크포인트, 멀티 시그널 관찰(exit code + stderr + 서버 헬스 + DOM snapshot).

#### Innovation 6: 구조적 관계 메모리 (Digital Twin Graph)

Typed Object/Link/Action 그래프 레이어가 사용자의 실세계를 디지털 트윈으로 모델링하며, 에피소드 메모리(SQLite FTS5 + vec) 위에 위치함. 그래프는 모든 성공적 행위 후 발화하는 deterministic rule engine 이 자동 유지 — LLM 오케스트레이션 없이 링크 생성, 객체 승격, 채널 프로파일링을 수행. E2E 암호화 동기화 프로토콜과 결합되어, 구조적 그래프가 모든 사용자 기기에 1-class 델타 연산으로 동기화됨.

---


### 13.3 구현 로드맵 (Implementation Roadmap)

<!-- CHUNK: 26_roadmap (원본 §15 Implementation Roadmap) -->

### 15. 구현 로드맵

#### 완료됨 (Completed)

- [x] ZeroClaw upstream 동기화 (1692 커밋 머지됨)
- [x] 도구 라우팅 포함 작업 카테고리 시스템 (7 카테고리)
- [x] 25 개 언어 지원 음성 파이프라인
- [x] 자동 VAD 포함 Gemini Live WebSocket 클라이언트
- [x] 동시 통역 세그멘테이션 엔진
- [x] 클라이언트-서버 통신용 WebSocket 이벤트 프로토콜
- [x] SimulSession 매니저 (오디오 전달 + 이벤트 처리)
- [x] 멀티 모델 코드 리뷰 파이프라인 (Gemini + Claude)
- [x] GitHub Actions Gemini PR 리뷰 워크플로
- [x] 코딩 샌드박스 6-phase 방법론
- [x] 프론트엔드용 Translation UI manifest
- [x] 크레딧 기반 과금 시스템
- [x] 아키텍처 문서 (본 문서)
...
### 13.4 Implementation Details Beyond Original Spec (Code-Verified)

<!-- CHUNK: 27_impl_details (원본 §15A Implementation Details Beyond Original Spec) -->

### 15A. 원본 스펙을 넘어선 구현 세부사항 (Code-Verified 2026-04-11)

> **번역 메모**: 본 부록은 파일 경로·함수명·상수명 등 사실성이 중요한 기술 상세가 대부분입니다. 오독 가능성을 최소화하기 위해 하위 섹션 **제목만 한국어로 번역**하고 본문의 구현 세부사항은 영문 원문을 그대로 유지합니다. 각 섹션의 요지는 제목과 불릿의 굵은 키워드로 파악할 수 있으며, 더 상세한 이해는 `src/` 하위 경로를 직접 참조하세요.
...
#### 15A.1 Smart API Key Routing — 추가 보안 장치
...
#### 15A.2 Channel Chat — 게이트웨이 보호 및 릴레이 강화
...
#### 15A.3 메모리 동기화 — 키 유도, 3-Tier 계층화, 온톨로지 동기화
...
#### 15A.4 Hot Memory Cache — 항상 캐시되는 지시사항 접두어
...
#### 15A.6 범용 이벤트 디스패치 서브시스템 (`src/dispatch/`)
...
### 13.5 For AI Reviewers

<!-- CHUNK: 28_for_ai_reviewers (원본 §16 For AI Reviewers) -->

### 16. AI 리뷰어용 안내

본 아키텍처에 대한 PR 리뷰 시 확인 항목:

1. **아키텍처 정합성 확인**: 변경이 trait-driven 패턴을 따르는가? 올바른 모듈에 속해 있는가?
2. **설계 원칙 확인**: KISS, YAGNI, SRP, fail-fast, secure-by-default.
3. **MoA 특정 계약 확인**: 음성 세그멘테이션 파라미터, 이벤트 프로토콜 호환성, 카테고리 도구 라우팅, 메모리 동기화 프로토콜.
4. **리스크 등급 확인**: 고위험 경로 (`security/`, `gateway/`, `tools/`, `workflows/`, `sync/`) 는 추가 검토 필요.
5. **하위 호환성 확인**: 설정 키는 공개 API — 변경 시 마이그레이션 문서 필요.
6. **플랫폼 독립성 확인**: 코드는 5 개 플랫폼 전부(Windows, macOS, Linux, Android, iOS) 에서 동작해야 함 — `cfg` 게이트 뒤가 아니라면 플랫폼 특정 가정 금지.
7. **메모리 동기화 계약 확인**: `memory/`, `sync/`, `ontology/` 에 대한 변경은 델타 기반·E2E 암호화·서버 비저장 불변성(invariant) 을 보존해야 함. 온톨로지 델타는 에피소드 메모리 델타와 동일한 프로토콜로 동기화.
8. **API key 처리 확인**: API key 를 절대 로깅 금지, 릴레이 서버로 절대 전송 금지, user-key 경로와 operator-key 경로를 모두 처리.
9. **통합 앱 계약 확인**: 사용자 관점에서 MoA 와 ZeroClaw 는 분리 불가능한 단일 앱 유지. 어떤 변경도 sidecar 아키텍처를 최종 사용자에게 노출해서는 안 됨(별도 설치 단계 금지, 사용자 UI 에 "ZeroClaw" 브랜딩 금지, 수동 프로세스 관리 금지). Sidecar IPC 오버헤드는 왕복당 1ms 미만 유지.
