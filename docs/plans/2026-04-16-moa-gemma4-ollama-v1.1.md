...
1. **Gemma 4 E2B/E4B 오디오 네이티브 입력** — §1, §4, §11에서 주장. 모델 차원의 오디오 인코더 실재는 Google 공식 발표로 뒷받침되나, **Ollama의 `/api/chat` 엔드포인트가 현재 `audio` 필드를 실제로 수용하고 오디오를 모델로 전달하는지**는 별도 확인 필요. 미지원 시 PR #6 재설계 (Whisper.cpp 로컬 백업 또는 직접 GGUF 로더 구동) 필요.
